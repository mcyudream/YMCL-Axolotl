...
function getCardActions(
	...
	const favoriteAction = isFavoriteContentType(currentProjectType)
...
async function search(requestParams: string, signal: AbortSignal) {
	...
	const hits = (rawResults?.result.hits ?? [])
		.map((hit) => {
			...
			if (activeInstance.value) {
				mapped.installed = allInstalledIds.value.has(hit.project_id)
			}
	...
	const directModrinthHits = rawDirectModrinth
		...
		.map((hit) => {
			if (activeInstance.value) {
				hit.installed = allInstalledIds.value.has(hit.project_id)
			}
			return hit
		})
...
const searchState = useBrowseSearch({
	...
	persistentQueryParams: ['i', 'ai', 'from', 'source'],
	getExtraQueryParams: () => ({
	...
	<div data-onboarding-id="browse-content" class="flex flex-col gap-3 p-6">
		<BrowsePageLayout v-if="!isWorldMapBrowse || curseForgeCapability.configured">
			...
			<template #search-bar-actions>
				<ButtonStyled
					v-if="projectType !== 'server' && projectType !== 'modpack'"
					size="standard"
					type="standard"
				>
