	...
	>
		<div v-if="hasPropertyControls" class="flex min-w-0 items-center gap-2">
			...
			<input
				...
				class="size-9 shrink-0 cursor-pointer rounded-xl border border-solid border-surface-5 bg-surface-4 p-1"
		...
		<Button type="base" :disabled="busy" @click="emit('save', 'replace_edit')">
			<SaveIcon aria-hidden="true" />
			{{ formatMessage(messages.overwrite) }}
		</Button>
	</div>
</template>
